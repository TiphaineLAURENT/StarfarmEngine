...
namespace star
{
    ...
    template <>
    struct Vector<2> : public cpVect
    {
        using cpVect::x;
        using cpVect::y;

        Vector(const Point<2> &tail) : Vector(tail.x, tail.y) {}

        Vector(const Coordinate &x_, const Coordinate &y_)
        {
            x = x_;
            y = y_;
        }

        Vector(const Vector<2>& copy) 
            : Vector(copy.x, copy.y)
        {}
        ...
        Vector<2> &operator*=(const Coordinate &scalar)
        {
            x *= scalar;
            y *= scalar;
        ...
        star::Vector<2> normalize() const
        {
            star::Length magnitude = calculate_magnitude();
            if (magnitude > 0)
                return star::Vector{ *this } /= magnitude;
            return *this;
        }

        Angle Vector<2>::calculate_direction() const { return std::atan(y / x); }
    };
