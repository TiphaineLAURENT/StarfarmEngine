...
namespace star
{
        ...
        void RigidbodyComponent::apply_force(Force fx, Force fy, Coordinate x, Coordinate y)
        {
                apply_force({ fx, fy }, { x, y });
        }
        void RigidbodyComponent::apply_force(const Vector2D &force, const Vector2D &point)
        {
        ...
        void RigidbodyComponent::apply_linear_impulse(Force fx, Force fy, Coordinate x, Coordinate y)
        {
                apply_linear_impulse({ fx, fy }, { x, y });
        }
        void RigidbodyComponent::apply_linear_impulse(const Vector2D &impulse, const Vector2D &point)
        {
                if (_type != RIGIDBODY_TYPE::DYNAMIC)
                {
                        return;
        ...
        void RigidbodyComponent::apply_linear_impulse_to_center(const Vector2D &impulse)
        {
                if (_type != RIGIDBODY_TYPE::DYNAMIC)
                {
                        return;
        ...
        const Weight &RigidbodyComponent::get_mass() const
        {
                return _mass;
        }
        ...
        void RigidbodyComponent::set_type(RIGIDBODY_TYPE type)
        {
                _type = type;
        }
        const RIGIDBODY_TYPE &RigidbodyComponent::get_type() const
        {
                return _type;
        }
        ...
        {
                return (_constraints & RIGIDBODY_CONSTRAINTS::FREEZE_ROTATION) == RIGIDBODY_CONSTRAINTS::FREEZE_ROTATION;
        }

        void RigidbodyComponent::allow_sleeping()
        {
                _allowSleep = true;
        }
        void RigidbodyComponent::disallow_sleeping()
        {
                _allowSleep = false;
        }

        bool RigidbodyComponent::is_sleeping_allowed() const
        {
                return _allowSleep;
        }
        ...
        std::vector<b2JointEdge> &RigidbodyComponent::get_joints()
        {
                return _joints;
        }
